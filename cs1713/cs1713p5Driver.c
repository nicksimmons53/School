...
Command Parameters:
    p5 -t teamFileName -c commandFileName
Input:
    Team	same as Programming Assignment #4 although there may be different data.
    ...
    Command	This is is in the form of commands: 
        ...
        TEAM NEW szTeamId  iWins  iLosses  dFeeAmount  dPaidAmount  
                    6s       d      d        lf          lf
             Add this new team to the binary tree.  Note that you should check whether 
...
{
    NodeT      *pRoot;                       // points to an ordered binary tree of teams
    ...
    // get the team and print them
    pRoot = getTeams(pszTeamFileName);
    printTeams("Initial Teams", pRoot);
    ...
    printTeams("Resulting Teams", pRoot);
    printf("\n");
    return 0;
}
...
Parameters:
    I/O NodeT **ppRoot                Address of the pointer to the root of
                                      an ordered binary tree of teams
...
void processCommands(NodeT **ppRoot, char *pszCommandFileName)
{
    FILE *pfileCommand;                     // stream Input for command processing
    ...
    */
    while (fgets(szInputBuffer, 100, pfileCommand) != NULL)
    {
        ...
        {   // GAME command
            bGotToken = getSimpleToken(szInputBuffer, &iBufferPosition, szSubCommand);
            if (bGotToken)
                processGameCommand(*ppRoot, szSubCommand
...
NodeT * getTeams(char * pszTeamFileName)
{
    ...
    */
    ...
    while (fgets(szInputBuffer, 100, pFileTeam) != NULL)
    {
        ...
        // Insert the team into the binary tree.  Since insertLL might
        // change the beginning of the binary tree, we must pass the
        // address of pRoot
       pRoot = insertT(pRoot, team);
    }
    fclose(pFileTeam);
    return pRoot;
}
...
Purpose:
    Allocates a new NodeT, placing the parameter in the NodeT.
Parameters:
    I   Team team    Team information for one team.
Returns:
    A pointer to the newly allocated NodeT.
Notes:
...
{
    NodeT *pNew;
    // to allocate a new NodeT
    pNew = malloc(sizeof(NodeT));
    if (pNew == NULL)
...
void exitUsage(int iArg, char *pszMessage, char *pszDiagnosticInfo)
{
    if (iArg >= 0)
        ...
        , pszMessage
        , pszDiagnosticInfo);
    fprintf(stderr, "p5 -t teamFileName -c commandFileName\n");
    if (iArg >= 0)
        exit(-1);
    else
        exit(-2);
