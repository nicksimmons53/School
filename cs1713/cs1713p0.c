 /********************************************************************************
cs1713p0.c by Nicholas Simmons
Purpose:
...
Input:
    ...
    Each line contains:
    dExam1 dExam2 dFinalExam szStudentNr szName
    10.2lf 10.2lf 10.2lf    6s   20s (but the name have blanks)
Results:
...
int main(int argc, char *argv[])
{
    ...
    { 
        // if the line is just a linfe feed, skip it.
        if (szInputBuffer[0] == '\n')
            continue;
    ...
    // Check for bad input. scan f returns the number of valid conversions
    if (iScanfCnt < 5)
    {
