...
void processTeamCommand(Node **ppHead, char *pszSubCommand, char *pszRemainingInput)
{
   ...
   // Determine what to do based on the SHOW subCommand
    else if (strcmp(pszSubCommand, "SHOW") == 0)
    {
        ...
        if (p < 0)
        {
            printf(WARN_TEAM_NOT_FOUND, p->team.szTeamId);
            printf("\n");
            return;
        }
    ...
    else if (strcmp(pszSubCommand, "NEW") == 0)
    {
        ...
        if (p != NULL)
            printf(WARN_TEAM_ALREADY_EXISTS, team.szTeamId);
    ...
    {
        // get the team info
        iScanfCnt = sscanf(pszRemainingInput, "%6[^,],%12[^,],%30[^,],%13[^,],%20s"
            , team.szTeamId
            , team.szTeamName
            , team.szEmailAddr
            , team.szPhone
        ...
        // Add the data to the appropriate node
    }
...
***********************************************************************************************/
    Node *insertLL(Node **ppHead, Team team)
    {
        ...
        // check if the the node already exists
...
Purpose:
    ...
    exists, it returns the pointer to the node that contains it. 
Parameters:
...
***********************************************************************************************/
    Node *searchLL(Node *pHead, char szMatchTeamId[ ], Node **ppPrecedes)
    {
        ...
        // Check to ensure node is NULL
        if (pHead == NULL || szMatchTeamId == pHead->team.szTeamId)
        {
            *ppPrecedes = NULL;
            return pHead;
        }
